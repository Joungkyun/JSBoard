<?php
##############################################################
# Copyright configuration
##############################################################
$designer['url'] = 'http://mywizz.com';
$designer['name'] = 'Wizz';
# 라이센스 관련 설정
# 2 - 라이센스 출력안함
# 1 - 라이센스 보존
# 0 - GPL
$designer['license'] = '1';
$designer['ver'] = 'A10';
##############################################################
# Language configuration
##############################################################
$langs['code'] = 'ko';		# 게시판 언어
$langs['font'] = '굴림체';	# 고정폭 폰트
$langs['vfont'] = '굴림';		# 가변폭 폰트

##############################################################
# Color configuration
##############################################################

# HTML 기본 색상 설정
$color['text']  = '#292929';      # TEXT
$color['n1_fg'] = '#FFFFFF';      # 사용 불가능

# 색상 설정
$color['nr_bg'] = '#D3DAC3';      # 공지글 배경 
$color['nr_fg'] = '#000000';      # 공지글 글자 
$color['l2_bg'] = '#FFFFFF';      # 보통글 배경
$color['l2_fg'] = '#292929';      # 보통글 글자
$color['l3_bg'] = '#FFFFFF';      # 답장글 배경
$color['l3_fg'] = '#000000';      # 답장글 글자
$color['l4_bg'] = '#ffffff';      # 폼 배경
$color['l4_fg'] = '#292929';	# 폼 글자
$color['l4_gu'] = '#292929';	# 폼 라인
$color['l5_bg'] = $color['l2_bg'];  # 글본문 배경
$color['l5_fg'] = $color['l2_fg'];  # 글본문 글자

# 검색한 문자열 색상
$board['hl']    = '<FONT STYLE="color:#6D7BC9;font-weight:bold;text-decoration:underline;">STR</FONT>';	

$color['ms_ov'] = '#F9F9F9';      # 마우스 오버
$color['td_co'] = '#336699';      # 오늘 올라온 글 표시
$color['cp_co'] = '#292929';      # 현재 페이지 표시

# 미리보기 색상 설정
$color['p_gu']  = '#999999';	# 미리보기 가이드 라인
$color['p_bg']  = '#FFFFFF';	# 미리보기 배경
$color['p_fg']  = '#555555';	# 미리보기 글자

# admin page 색상 설정
$color['b_bg'] = '#FFFFFF';	# 바탕 색상
$color['t_bg'] = '#292929';	# 제목줄 배경
$color['t_fg'] = '#FFFFFF';	# 제목줄 글자
$color['m_bg'] = '#999999';	# 메뉴칸 배경
$color['m_fg'] = '#FFFFFF';	# 메뉴칸 배경
$color['d_bg'] = '#EFEFEF';	# 설명칸 배경
$color['d_fg'] = '#555555';	# 설명칸 배경

# 글 리스트 tr 의 높이 설정
# list table 에서는 image 의 height 로 조정. 예를 들어
# <IMG SRC=img/blank.gif WIDTH=0 HEIGHT=$line_height BORDER=0>
$lines['height'] = '22';

# 글 리스트들 사이에 디자인을 넣기위한 코드 이설정은 <TD></TD>
# 설정으로 이루어 짐. <TD> 에는 꼭 COLSPAN=AA 가 들어가야 함. 
# 이설정은 리스트의 colspan 을 맞추기 위해 list.php 에서 변환을
# 하게 되어 있음 다음의 예제를 이용하여 테스트 해 보기 바람
$lines['design'] = '<TD COLSPAN="AA" BGCOLOR="#FFFFFF" STYLE="background-image: url(theme/KO-wizz/img/vertline.gif);">'.
                 '<IMG ALT="" SRC="./images/blank.gif" WIDTH=1 HEIGHT=1 BORDER=0></TD>';

##############################################################
# Width Configuration
##############################################################

# list paget table ratio ===================================
$td_width['no'] = '5%';           # 번호 필드 너비

# 제목 필드 너비
if ($upload['yesno']) $td_width['title'] = '61%';
else $td_width['title'] = '70%';

$td_width['name']   = '11%';              # 글쓴이 필드 너비
$td_width['upload'] = '6%';               # 파일 필드 너비
$td_width['dates']  = '13%';              # 날짜 필드 너비
$td_width['refer']  = '5%';               # 읽은수 필드 너비

##############################################################
# Field Array Configuration
##############################################################
# 글 리스트의 필드 순서를 정의한다.
# n  -> 글번호
# T  -> 글제목
# N  -> 글쓴이
# F  -> 업로드 파일
# D  -> 글등록 날자
# R  -> 읽은 수
#
$td_array = 'nTNFDR';
?>
