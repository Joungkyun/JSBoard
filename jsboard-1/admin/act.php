...
if( $mode != 'manager_config') {
  ...
  if($mode == 'db_create')  {
    ...
    $insert_data = "INSERT INTO {$new_table}
                         VALUES (
                           '',1,1,{$date},'{$host_ext}','{$name_ext}','{$passwd_ext}',
    ...
    copy ("../Installer/sample/{$ostypes['name']}/config.ph",
          "../data/{$new_table}/config.ph");
    chmod("../data/{$new_table}/config.ph",0644);
    copy ("../Installer/sample/{$ostypes['name']}/html_head.ph",
          "../data/{$new_table}/html_head.ph");
    chmod("../data/{$new_table}/html_head.ph",0644);
    copy ("../Installer/sample/{$ostypes['name']}/html_tail.ph",
          "../data/{$new_table}/html_tail.ph");
