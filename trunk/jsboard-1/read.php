...
if ($o['sc']) {
  $list['text'] = preg_replace(
                    '!<a href=(.*)<font color=#[0-9a-z]+><b><u>(.*)</u></b></font>(.*) target=(.*)>!i',
                    "<a href=\"\\1\\2\\3\" target=\"\\4\">",
                    $list['text']
  );
}
...
if ($board['cmd'] == 'yes' && $board['img'] != 'yes') {
  ...
  if($enable['dwho']) $langs['hlinked'] = "<a href=\"#\" onclick=\"new_windows('./whois.php?table={$table}&host={$list['host']}',0,1,0,600,480); return false\"><font color=\"{$color['text']}\">{$list['dhost']}</font></a>";
...
echo <<<EOF
<TD valign="top" BGCOLOR="{$color['r0_bg']}">
...
if($list['bofile']) {
  ...
  <A HREF="{$down_link}"><IMG SRC="images/{$fileicon}" width="16" height="16" border="0" alt="{$list['bofile']}">
  <FONT COLOR="{$color['r4_fg']}" {$board['css']}>{$list['bofile']}</FONT></A>
...
if($board['img'] == 'yes' && $color['bgcol'] == $color['r5_bg'])
  echo '<TD>&nbsp;</TD>';
